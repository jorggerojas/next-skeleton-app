...
- **Export types from type files** - import from `@/types/*`
...
- **api-routes** (scope: app-router): Create and work with Next.js Route Handlers in src/app/api/. Use when creating API endpoints, handling HTTP requests, or working with server-side API logic.
- **app-router** (scope: hooks, stores, components-ui): Work with Next.js App Router in src/app/. Use when creating or modifying pages, layouts, loading states, error boundaries, or any route components.
- **components-ui** (scope: stores, testing): Create and organize UI components in src/components/. Use when creating new custom components, organizing component structure, or working with component exports.
- **hierarchy** (scope: components-ui, hooks, stores, app-router): Define the components hierarchy, how to use components inside pages and how to mix components and when it's needed to create new ones.
- **hooks** (scope: testing): Create and use custom React hooks in src/hooks/. Use when working with React and want to implement some repetitive functions or extract all the "weight" from a component into custom functions.
...
- **schemas** (scope: components-ui, hooks, app-router, testing): Define the form schemas with yup, how to implement them and where to place them, how to type and how not.
